...
with a row for each team, with average statistics for all games played in that time period. This reflects the behavior of the NBA stats 
...
![Player Positions - Wikipedia](data/Basketball_Positions.png)


We turned to [Basketball Reference](https://www.basketball-reference.com/players) for more complete player profiles. This data is collected by a parent organization that manages 
...
This data goes back all the way to 1951, near the inception of professional basketball.
...
slightly unequal by +/- a few games played from team to team. The solution to this would be to obtain averages based 
...
we are looking for differences in only general performance.

Inconsistency was observed in the naming conventions of the basketball positions. E.g. the position 
scraped for a player was observed to be Guard, but on a study of that players position, it was found out that he played Point Guard. Many players have a nicknames/unofficial names that a few of the data 
sources use. Hence, keeping track of player information while subsetting and filtering out data was an issue.
