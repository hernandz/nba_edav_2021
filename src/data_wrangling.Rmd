...
season_2019_players_playoffs <- teams_players_stats(seasons = 2019, measures = "Advanced",  season_types = c("Playoffs"), types = "player", tables = "general", modes = "PerGame", starters_bench = list("Starters", "Bench")) %>% 
  unnest(dataTable) %>% 
  mutate(df_type = "playoffs")
...
bind_rows(
  ...
  filter(!(namePlayer %in% c("Luc Mbah a Moute", "Jabari Parker"))) %>% #waived
  mutate(ordered_names = fct_reorder(str_c(namePlayer, " ", date_reported), date_reported, .desc = FALSE)) %>% 
