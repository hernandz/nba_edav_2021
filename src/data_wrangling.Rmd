...
covid_season_shots <- teams_players_stats(seasons = 2020, measures = "Advanced", types = "team", tables = "general", modes = "PerGame", date_from = "10-22-19", date_to = "3-11-20")
covid_season_unlisted <- covid_season$dataTable[[1]] %>% 
  mutate(df_type = "all_before")
...
#          ) %>% 
#  semi_join(bubble_unlisted, by = "nameTeam") %>% 
#  mutate(ordered_teams = fct_reorder2(nameTeam, df_type == "all_before", ortg, .desc = FALSE)) %>% 
...
season_2017_2019 <- teams_players_stats(seasons = 2017:2019, measures = "Advanced", season_types =  c("Regular Season", "Playoffs"), types = "team", tables = "general", modes = "PerGame")
...
reg_2019 <- season_2017_2019$dataTable[[3]] %>% 
  mutate(df_type = "regular_season")
reg_2019_last_8 <- season_2017_2019_last_8$dataTable[[1]] %>% 
  mutate(df_type = "regular_season_last_8")
playoffs_2019 <- season_2017_2019$dataTable[[6]] %>% 
...
reg_2019_season_players <- teams_players_stats(seasons = 2019, measures = "Advanced", types = "player", tables = "general", modes = "PerGame")
reg_2019_season_players_unlisted <- reg_2029_season_players$dataTable[[1]] %>% 
  mutate(df_type = "all_before")


reg_2019_season_players_last_8 <- teams_players_stats(seasons = 2019, measures = "Advanced",  types = "player", season_types = c("Regular Season", "Playoffs"), tables = "general", modes = "PerGame", last_n_games = 8)

reg_2019_season_players_last_8_unlisted <- reg_2019_season_players_last_8$dataTable[[1]] %>% 
  mutate(df_type = "last_8") 


bind_rows(covid_season_players_unlisted,
          bubble_players_unlisted, 
          #reg_2019_season_players_last_8_unlisted
          ) %>% 
...
bind_rows(#covid_season_month_before_unlisted,
          reg_2019_season_players_unlisted, 
          reg_2019_season_players_last_8_unlisted
          ) %>% 
    filter(namePlayer %in% c("Marcus Smart",
                           "Rudy Gobert",
