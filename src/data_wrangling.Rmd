...
season_2019_players <- teams_players_stats(seasons = 2019, measures = "Advanced", types = "player", tables = "general", modes = "PerGame") %>% 
  unnest(dataTable) %>% 
  mutate(df_type = "all_before")
...
bind_rows(#covid_season_players,
   ...
   #                        "Ricky Rubio",
   #                        "Kendrick Nunn")) %>% 
  inner_join(covid_players_fixed, by = c("namePlayer" = "name")) %>% 
  filter(!(namePlayer %in% c("Luc Mbah a Moute", "Jabari Parker"))) %>% 
  mutate(ordered_names = fct_reorder(namePlayer, date_reported, .desc = FALSE)) %>% 
  ggplot(aes(x = ordered_names, y = netrtg, color = df_type)) + 
  geom_point(alpha = .7) + 
  theme(axis.text.x = element_text(angle = 90))

#mo bamba hurt at end of season
bind_rows(#season_2019_players, 
...
starters_top_guys <- teams_players_stats(seasons = 2020, types = "player", starters_bench = list("Starters")) %>% 
  unnest(dataTable) %>% 
...
covid_season_players %>% 
  semi_join(bubble_seeding_players, by = "namePlayer") %>% 
  semi_join(starters_top_guys, by = "namePlayer") %>% 
  group_by(slugTeam) %>% 
  summarize(count = n())
...
covid_season_players_month_before %>% 
  ...
  semi_join(starters_top_guys, by = "namePlayer") %>% 
  ggplot(aes(x = drtg, y = ortg, color = df_type)) + 
  geom_point(alpha = .7) + 
  scale_x_reverse() + 
  facet_wrap(~slugTeam)
