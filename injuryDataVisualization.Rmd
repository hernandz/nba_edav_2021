...
```{r}
vcd::mosaic(typeInjury ~ position1,spacing = vcd::spacing_equal(sp = unit(0, "lines")),labeling = vcd::labeling_border(rot_labels = c(90, -90, 0, 0),highlighting_fill = scale_colour_distiller,gp_labels = gpar(fontsize = 6),gp_varnames = gpar(fontsize = 14, fontface = 1),pos_labels = "center",just_labels = "center",shade=T, colorize = T,gp = gpar(fill=matrix(c("red","blue","light green", "green", "black","red","brown","white","blue"), 3, 3))),injuryData_2017)
```
...
linePlotDataFrame %>% ggplot( aes(x=year,y=n, group=position1, color=position1)) +geom_line() +geom_point()
```
...
treemap(treeMapDataFrame,index = c('year','typeInjury'),vSize = 'n',type = 'index')
```
