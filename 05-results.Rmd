...
As previously explained, we focused on the Offensive/Defensive/Net ratings to evaluate the way they played. A Cleveland dot plot 
sorted by ratings before the Bubble could allows to easily see improvement or lag in performance. Dots on the left of the blue line of 
dots would mean worse, to the right, better.
...
bubble_stats_long %>% 
  ...
  ylab(NULL) + 
  ggtitle('NBA team ratings before/during the 2020 COVID Bubble - Defense')
```
...
bubble_stats_long %>% 
  ...
  ylab(NULL) +  
  ggtitle('NBA team ratings before/during the 2020 COVID Bubble - Net')
...
season_19_long %>% 
  ...
  xlab("Offensive Rating") + 
  ylab(NULL) + 
  ggtitle('NBA team ratings near the end of the 2019 season - Offense')
...
season_19_long %>% 
  ...
  xlab("Net Rating (Offensive - Defensive)") + 
  ylab(NULL)+ 
  ggtitle('NBA team ratings near the end of the 2019 season - Net')

```
  
### Players

If the COVID Bubble did not impact team performance, what about COVID itself? A number of 
specific players were reported to have contracted the virus during the suspension of the season. 
Most of these players eventually came back to play in the Bubble. We tried to see 
...
Note: not every player that contracted COVID and played in the Bubble had game data 
from near the end of the previous season. These names were intentionally left in to 
make comparison between both graphs more convenient.
```{r echo = FALSE, message = FALSE, results='hide'}
covid_players <- read_csv("data/covid_players.csv")
...
covid_players_fixed %>%
  ...
  scale_color_manual(values = c("month_before" = "#00bcd4",
                              "bubble_seeding" = "#ff9800", 
                              "bubble_playoffs" = "#e91e63"),
                     labels = c("Month before Bubble",
                              "Bubble seeding games",
                              "Bubble playoffs"),
                     name = "Time Period") +
  xlab("Players") + 
  ylab("Net Rating") + 
  ggtitle('NBA player ratings before/during the 2020 COVID Bubble')
...
covid_players_fixed %>%
  ...
  ylab("Net Rating") + 
  ggtitle('NBA player ratings near the end of the 2019 season')
...
```{r echo = FALSE, message = FALSE, results='hide'}
ggplot(pivoted_bar_plot_df_2020_top_teams, aes(x=teams, y=value, fill=variable)) +
  geom_bar(stat='identity', position='dodge')+
  coord_flip()+
  ggtitle("Performance Comparision of Top 15 Teams",
          subtitle = "Before and After Event (Death)") + 
    scale_fill_discrete(labels = c("Before death",
                              "After death"),
                     name = "Time Period") + 
  xlab("Teams") + 
  ylab("Net Rating")

ggplot(pivoted_bar_plot_df_2020_bottom_teams, aes(x=teams, y=value, fill=variable)) +geom_bar(stat='identity', position='dodge')+coord_flip()+ggtitle('Performance Comparision of Bottom 15 Teams',
subtitle = 'Before and After Event (Control)') +
  scale_fill_discrete(labels = c("Before 26th",
                              "After 26th"),
                     name = "Time Period") + 
  xlab("Teams") + 
  ylab("Net Rating")
...
The Utah Jazz to show a performance dip. This may or may not be due to the loss of Kobe Bryant, since the downtrend began in the previous season. Moreover, Bryant didn't have much of an association with the Jazz (apart from the fact that he played his final Lakers game against them).
...
The list of players was obtained from [A celebrity entertainment website](https://www.thethings.com/15-nba-stars-kobe-bryant-was-close-to/). The players obtained were 'Carmelo Anthony','LeBron James','Anthony Davis','Trae Young','Kevin Durant','Stephen Curry' and 'Chris Paul'. However, Steph Curry and Kevin Durant did not play during this period in 2020 so they were omitted. Trae Young and Anthony Davis were also the only two to have a 
control to compare to.


```{r echo = FALSE, message = FALSE, results='hide'}
kobe_bryant_friends <- c('Carmelo Anthony','LeBron James','Anthony Davis','Trae Young','Chris Paul')
...
ggplot(consolidated_player_data, aes(x=namePlayer, y=netrtg, fill=status)) +
  geom_bar(stat='identity', position='dodge')+facet_wrap(~year,ncol=1)+coord_flip()+ggtitle('Players vs. Net Rating') +
    scale_fill_discrete(labels = c("Before death",
                              "After death"),
                     name = "Time Period") +
  scale_fill_discrete(labels = c("Before 26th",
...
From the above graph, it could be said that the demise didn't affect most players. Chris Paul is one such player who showed a drop in performance after the 26th of January. This maybe possible due to Kobe's death because Chris Paul was really close to Bryant (both of them represented USA in the Olympics). Moreover, the sports media has highlighted their friendship a plenitude of times.
...
Note: we really struggled to get a better output for these labels! 
```{r echo = FALSE, message = FALSE,  results='hide', fig.width=9}
...
injuryData_2017 %>% 
  rename(Position = "position1",
         Injury = "typeInjury") %>% 
vcd::mosaic(Injury ~ Position,main = 'Injury vs Position: 2017',spacing = vcd::spacing_equal(sp = unit(0, "lines")),labeling = vcd::labeling_border(rot_labels = c(90, -90, 0, 0),gp_labels = gpar(fontsize = 10),gp_varnames = gpar(fontsize = 14, fontface = 1),pos_labels = "center",just_labels = "center"),.,highlighting_fill = brewer.pal(n = 8, 'Blues'))
```
...
linePlotDataFrame %>% ggplot( aes(x=year,y=n, group=position1, color=position1)) +
  geom_line() +
  geom_point()+
  ggtitle('Number of Injuries over the years by playing position') + 
  xlab("Year") + 
  ylab("Count") +
...
treemap(treeMapDataFrame,index = c('year','typeInjury'),vSize = 'n',type = 'index',title="Injury types over the years")
```
